...
INSERT INTO comment (comment_id, parent_id, bug_id, author, comment_date, comment)
VALUES (1, null, 1, 1, now(), '이 버그의 원인이 뭘까'),
       (2, 1, 1, 2, now(), '널 포인터 때문인 것 같아'),
       (3, 2, 1, 1, now(), '아니, 그건 확인해봤어'),
       (4, 1, 1, 3, now(), '입력 값이 유효한지 확인'),
       (5, 4, 1, 2, now(), '그래, 그게 버그야'),
       (6, 4, 1, 1, now(), '그래 확인 코드 추가'),
       (7, 6, 1, 3, now(), '수정했어');
...
DELETE
FROM comment
WHERE parent_id IN (7);
DELETE
FROM comment
WHERE parent_id IN (5, 6);
DELETE
FROM comment
WHERE parent_id IN (4);
